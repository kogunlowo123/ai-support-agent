...
REPOSITORY_URL: Final[str] = "https://github.com/kogunlowo123/ai-support-agent"
...
<div class="layout">
<nav class="sidebar">
<h1><a href="index.html">AI Support Agent</a></h1>
...
def build(output: Path) -> int:
    ...
    tagline = "A bounded agent: enforced tool authority, computed policy, verified answers."
    written = 0
    for page in PAGES:
        ...
        (output / page.output_name).write_text(
            TEMPLATE.format(
                title=html.escape(
                    "AI Support Agent"
                    if page.slug == "index"
                    else f"{page.title} — AI Support Agent"
                ),
